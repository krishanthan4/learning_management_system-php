...
if (isset($_SESSION["admin_lms"])) {
  ...
  <div class="text-blueGray-700 antialiased w-full">
    ...
    $student = $student_rs->fetch_assoc();
     $teacher_rs = Database::search("SELECT COUNT(`email`) AS teacher_count FROM `user` WHERE `positions_id`='3'");
     $teacher = $teacher_rs->fetch_assoc();
    $academic_officers_rs = Database::search("SELECT COUNT(`email`) AS academic_officers_count FROM `user` WHERE `positions_id`='4'");
