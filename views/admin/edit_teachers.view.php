...
if (isset($_GET["id"])) {
  ...
  $editAdmin_rs = Database::search("SELECT * FROM `learning_management_system`.`teacher` 
INNER JOIN `address` ON `teacher`.`address_id`=`address`.`address_id` 
INNER JOIN `city` ON `city`.`id`=`address`.`city_id`
 ...
 WHERE `teacher`.`teacher_id`=?", [$_GET["id"]]);
          ...
          <div class="space-y-6 sm:px-6 lg:px-0 lg:col-span-9">
            <div class="max-w-7xl mx-auto sm:px-2 lg:px-8">
              <div class="max-w-2xl  mx-auto px-4 lg:max-w-4xl lg:px-0">
                <h1 class="text-2xl font-extrabold tracking-tight text-gray-900 sm:text-3xl">Edit teacher</h1>
                ...
                <div class="shadow sm:rounded-md sm:overflow-hidden">
                  <div class="bg-white py-6 px-4 sm:p-6">
                    <div class="mt-6 grid grid-cols-4 gap-6">
                      ...
                      <div class="col-span-4 sm:col-span-2">
                        <label class="block text-sm font-medium text-gray-700">Class In Charge (optional)</label>
                        <select id="classPosition"
                          class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-gray-900 focus:border-gray-900 sm:text-sm">
                          <option>Select</option>
                          <?php
                          $class_rs = Database::search("SELECT * FROM `class`");
                          $teacher_has_class_rs = Database::search("SELECT * FROM `teacher_has_class`");
                          for ($x = 0; $x < $class_rs->num_rows; $x++) {
                            $class_data = $class_rs->fetch_assoc();
                            $teacher_has_class = $teacher_has_class_rs->fetch_assoc();
                            ?>
                            <option value="<?php echo $class_data["id"]; ?>" <?php
                               if (!empty($teacher_has_class["class_id"])) {
                                 if ($class_data["id"] == $teacher_has_class["class_id"]) {
                                   ?>selected<?php
                                 }
                               }
                               ?>>
                              <?php echo $class_data["grade"] . " - " . $class_data["class_name"]; ?>
                            </option>
                            <?php
                          }
                          ?>
                      ...
                      <!-- subject -->
                      <div class="col-span-4 sm:col-span-2">
                        <label class="block text-sm font-medium text-gray-700">Teaching Sujects</label>
                        ...
                        <div class="flex gap-1 flex-row">
                          <select id="position"
                            ...
                            <option>Select</option>
                            <?php
                          $teacher_has_subject_rs = Database::search("SELECT * FROM `subject` INNER JOIN `teacher_has_subject` ON `teacher_has_subject`.`subject_id`=`subject`.`id` WHERE `teacher_id`='" . $editAdmin["teacher_id"] . "'");
                          // $teacher_has_subject1 = $teacher_has_subject_rs->fetch_assoc();


                            $subject_rs = Database::search("SELECT * FROM `subject`");
                            for ($x = 0; $x < $subject_rs->num_rows; $x++) {
                              $subject_data = $subject_rs->fetch_assoc();
                              ?>
                              <option value="<?php echo $subject_data["id"]; ?>">
                          ...
                          <button class="bg-green-500 text-white hover:bg-green-600 px-2 rounded-md" onclick="addSubject('<?= $editAdmin['teacher_id'] ?>');">Add</button>
                        </div>

                        <ul id="subjectUL" class="ps-3 pt-2" style="list-style-type:disc;">
                          <?php
                        if($teacher_has_subject_rs->num_rows!==0){
                          for ($w = 0; $w < $teacher_has_subject_rs->num_rows; $w++) {
                            $teacher_has_subject = $teacher_has_subject_rs->fetch_assoc();
                            ?>
                            <li class="border-b rounded-md border-1 ps-1 py-1">
                              <?php
                             echo $teacher_has_subject["subject"];
                              ?>
                            </li>
                            <?php
                          }
                ...
                <div class="shadow sm:rounded-md sm:overflow-hidden">
                  ...
                  <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
                    <button
                      ...
                      onclick="editAdministrator('<?= $editAdmin['teacher_id'] ?>');">Save</button>
                  </div>
                </div>
...
<!--alert model -->
<script src="/public/js/edit_teachers.js"></script>
